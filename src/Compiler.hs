...
convAst (Call "?" (ast1:ast2:_)) = (loopAst [ast2]) ++ (loopAst [ast1]) ++ "CALL EQUAL\n"
convAst (Call "<" (ast1:ast2:_)) = (loopAst [ast2]) ++ (loopAst [ast1]) ++ "CALL LESS\n"
convAst (Call "if" (ast1:ast2:ast3:_)) = let cond = loopAst [ast1]
                                             trueCond = loopAst [ast2] ++ "JUMP " ++ (show $ myLength falseCond) ++ "\n"
